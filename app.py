...
BACKUP_SETTINGS = {
    ...
    'yandex_token': 'y0__xDz4I2SBhjblgMgrMmc9xKzyBd8rJf61EkOfN-RjbLE7caQeA',  # OAuth-токен Яндекс.Диска
    'last_backup': None,
...
ALLOWED_CITIES = [
"Агачаул", "Аданак", "Гели", "Губден", "Гурбуки", "Джанга", "Доргели", "Зеленоморск", "Какамахи", "Кака-Шура", "Карабудахкент", "Ленинкент", "Манаскент", "Параул", "Сираги", "Уллубийаул",
  "Ачи-Су", "Манас",
  "агачаул", "аданак", "гели", "губден", "гурбуки", "джанга", "доргели", "зеленоморск", "какамахи", "кака-шура", "карабудахкент", "ленинкент", "манаскент", "параул", "сираги", "уллубийаул",
  "ачи-су", "манас", "манаскент", 
]
...
def get_ticket_number():
    """Получение номера участника из сессии или по номеру телефона"""
    # Проверяем наличие параметра phone в запросе
    phone = request.args.get('phone')
    ...
    if phone:
        ...
        # Проверяем, что номер телефона полный (не менее 11 цифр для российского номера)
        if len(normalized_phone) < 11:
            return jsonify({'success': False, 'message': 'Пожалуйста, введите полный номер телефона.'}), 400
        ...
        # Если телефон не указан, берем номер из сессии
...
def admin_panel():
    """Админ-панель"""
    # Загружаем свежие данные о участниках для получения статистики
    ...
    stats = {
        ...
        'male': sum(1 for p in participants if p.get('gender') == 'male'),
        'female': sum(1 for p in participants if p.get('gender') == 'female')
    }
    ...
    while True:
        ...
        if current_time >= next_time:
            ...
            if interval == 'custom':
                unit = backup_settings.get('custom_unit', 'hours')
                if unit == 'seconds':
                    # Для секунд проверяем каждые 10 минут вместо каждую секунду
                    wait_seconds = 600
                elif unit == 'minutes':
                    # Для минут проверяем каждые 10 минут вместо каждых 5 секунд
                    wait_seconds = 600
                else:
...
def admin_participants():
    """Страница просмотра участников"""
    # Загружаем свежие данные о участниках, игнорируя кэш
    all_participants = load_participants(force_reload=True)
    
    # Получаем параметр страницы из запроса
    page = request.args.get('page', 1, type=int)
    
    # Настройка пагинации
    per_page = 100  # Количество участников на странице
    total_participants = len(all_participants)
    total_pages = (total_participants + per_page - 1) // per_page if total_participants > 0 else 1
...
@app.route('/check-coordinates-with-city')
def check_coordinates_with_city():
    """Проверка местоположения пользователя по координатам с возможностью указать город вручную"""
    lat = request.args.get('lat')
    lng = request.args.get('lng')
    city_input = request.args.get('city', '').lower().strip()
    
    result = {"status": "error", "message": "Не удалось определить местоположение"}
    ...
    if lat and lng:
        try:
            ...
            app.logger.error(f"Ошибка при проверке координат: {str(e)}")
    
    # Если координаты не дали результата и есть ручной ввод города
    if result["status"] == "error" and city_input:
        # Проверяем введенный вручную город
        allowed = check_location_allowed(city_input)
        result = {
            "status": "success", 
            "allowed": allowed,
            "city": city_input,
    ...
    # Если нет результата от координат и нет ручного ввода, используем IP
    if result["status"] == "error":
        ip_address = request.remote_addr
        # Для локальной разработки
        if ip_address == '127.0.0.1':
            return jsonify({"status": "success", "allowed": True, "city": "махачкала (тестовый режим)", "source": "test"})
        ...
        try:
            ...
            app.logger.error(f"Ошибка при проверке IP: {str(e)}")
